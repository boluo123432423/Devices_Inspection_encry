...
def decrypt_oralce(message,key_pri):
    ...
    try:
        ...
        print(f'解密失败，请检查key和密文是否正确,密文：{str(message)}   key:{str(key_pri)}')
        print('将使用密文尝试登录...')
        return message


def get_devices_info(info_file,key_pri):  # 获取信息文件中的设备登录信息
    try:
        ...
        for device in devices_dict:
            password_value = device.get('password', None)
            secret_value = device.get('secret', None)

            if password_value is not None:
                device['password'] = decrypt_oralce(password_value, key_pri)
            if secret_value is not None:
                if str(secret_value) != 'nan':
                    device['secret'] = decrypt_oralce(secret_value, key_pri)
...
if __name__ == '__main__':
    ...
    devices_info = get_devices_info(INFO_PATH,key)  # 读取所有设备信息
